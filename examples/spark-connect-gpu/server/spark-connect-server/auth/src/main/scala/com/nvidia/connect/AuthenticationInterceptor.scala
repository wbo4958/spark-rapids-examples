...
class AuthenticationInterceptor extends ServerInterceptor {
  ...
  private val authorizationMetadataKey = Metadata.Key.of("Authorization", Metadata.ASCII_STRING_MARSHALLER)

  // Read file and convert to Map[String, String]
  private val userTokens: Map[String, String] = Using.resource(Source.fromFile(CREDENTIALS_FILE)) { source =>
    source.getLines()
      .filter(_.contains(":"))
      .map { line =>
  ...
  override def interceptCall[ReqT, RespT](call: ServerCall[ReqT, RespT],
    ...
    val authHeaderValue = Option(metadata.get(authorizationMetadataKey)).getOrElse {
      call.close(Status.UNAUTHENTICATED.withDescription("No authentication token provided"), new Metadata())
                    ...
                    case Some(expectedToken) =>
                      if (expectedToken != authHeaderValue) {
                        call.close(Status.UNAUTHENTICATED.withDescription(
                          "Invalid authentication token"), new Metadata())
                      }
                    case None =>
                      call.close(Status.PERMISSION_DENIED.withDescription(
